                                         ...
                                         int gwId)
{
  ...
  LoraPacketTracker::CountAverageDelay (Time startTime, Time stopTime)
  {
    ...
    double received = 0;
    double avgdelay = 0;
    std::vector<int> SFDelayCounts (6, 0);
    ...
    for (auto it = m_macPacketTracker.begin ();
         ...
         ++it)
      {
        if ((*it).second.sendTime >= startTime && (*it).second.sendTime <= stopTime)
          {
            st = (*it).second.sendTime.GetSeconds();
            sent++;
            if ((*it).second.receptionTimes.size ())
              {
                auto last = (*it).second.receptionTimes.end();
                --last;
                rt = (*last).second.GetSeconds();
                delai = delai + (rt - st);
                received++;
              }
          }
    ...
    if (received > 0)  {  avgdelay = delai / received;  }
